...
const validateTeamName = async (req, res, next) => {
  ...
  let teams = await selectQuery('team-names-lower', 'lower(attacker)', true, 'map_stats', null, null, 'lower(attacker) ASC')
...
const validateDualTeams = async (req, res, next) => {
  ...
  if (!teams.includes(teamOne.toLowerCase()) || !teams.includes(teamTwo.toLowerCase())) {
...
teamsRouter.get('/:team/matches', validateTeamName, async (req, res) => {
  ...
  if (!req.query.year) {
    ...
    for (const year of years) {
      const yearlyMatches = await selectQuery(`${team}-${year}-match-history`, `stage, 
    ...
    const results = await selectQuery(`${team}-match-history`, `stage, 
    CASE WHEN playoffs IS NULL AND postseason IS NULL THEN 'Regular Season' 
    WHEN playoffs = 'true' AND postseason IS NULL THEN 'Stage Playoffs' 
    WHEN playoffs = 'true' AND postseason = 'true' THEN 'Postseason' 
    END AS stage_type,
...
teamsRouter.get('/:team/match-history', validateTeamName, async (req, res) => {
  ...
  if (!req.query.year) {
    const results = {}
    for (const year of years) {
      const yearlyMatches = await selectQuery(`${team}-${year}-match-history-detailed`,
      ...
      MAX(match_winner) as match_winner`, false, 'map_stats',
      [
        ['year = ', year],
      ...
      ], 'stage, playoffs, postseason, match_id', 'stage ASC, playoffs DESC')

      for (const match of yearlyMatches) {
        if (match['matchup'][0].length === 2) { //eslint-disable-line
    ...
    const results = await selectQuery(`${team}-match-history-detailed`,
    `stage, CASE WHEN playoffs IS NULL AND postseason IS NULL THEN 'Regular Season' 
    WHEN playoffs = 'true' AND postseason IS NULL THEN 'Stage Playoffs' 
    WHEN playoffs = 'true' AND postseason = 'true' THEN 'Postseason' 
    END AS stage_type, match_id, ARRAY_AGG(DISTINCT ARRAY[attacker, defender]) AS matchup,
    ...
    if (results.length > 0) {
      ...
      res.status(404).send('No matches found for the given year. Is this an expansion team?')
    }
  }
})
